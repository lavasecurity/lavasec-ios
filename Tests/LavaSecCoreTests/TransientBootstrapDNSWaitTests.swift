...
/// (Phase E2; migrated to a dispatch-backed actor in actors slice 3): the
/// 64-deep / 4 s bounds and the generation/expired-generation transitions the
/// provider's source pins could only assert as text. Every exit except a
/// committed current-lifecycle snapshot returns the queue for SERVFAIL; queued
/// requests are only ever released for replay through the filter. Calls drive
/// the actor exactly like the provider's dnsStateQueue-confined regions:
/// already on the confinement queue, synchronous `assumeIsolated` — no hops.
final class TransientBootstrapDNSWaitTests: XCTestCase {
    private let queue = DispatchSerialQueue(label: "transient-bootstrap-wait-tests")
    ...
    /// cancelled `asyncAfter` work item. Lock-boxed — the `@Sendable`
    /// scheduleAfter seam forbids unsynchronized mutable state (same pattern as
    /// DeviceDNSCaptureRetryCycleTests' ManualClock).
    private final class TimeoutSpy: @unchecked Sendable {
        private let lock = NSLock()
        private var rawArmedIntervals: [TimeInterval] = []
        private var rawArmedItems: [DispatchWorkItem] = []

        var armedIntervals: [TimeInterval] {
            lock.lock(); defer { lock.unlock() }
            return rawArmedIntervals
        }

        var armedItems: [DispatchWorkItem] {
            lock.lock(); defer { lock.unlock() }
            return rawArmedItems
        }

        func schedule(after interval: TimeInterval, body: @escaping @Sendable () -> Void) -> DispatchWorkItem {
            let item = DispatchWorkItem(block: body)
            lock.lock()
            rawArmedIntervals.append(interval)
            rawArmedItems.append(item)
            lock.unlock()
            return item
        }
    ...
    func testTimeoutExpiryDrainsEverythingSERVFAILBound() {
        let (wait, spy) = makeWait()
        // Lock-boxed: the timeout handler is `@Sendable` (it crosses the injected
        // scheduler seam), so it cannot capture a mutable local.
        let drained = LockedResult<[Int]?>(nil)
        queue.sync {
            wait.assumeIsolated { wait in
                _ = wait.beginWait(generation: 3) { generation in
                    // Mirrors the provider's timeout handler: delivered on the
                    // confinement queue (the scheduleAfter contract), it re-enters
                    // isolation via assumeIsolated and marks the generation expired
                    // so latecomers keep SERVFAILing.
                    wait.assumeIsolated { wait in
                        drained.value = wait.fail(expectedGeneration: generation, marksGenerationExpired: true)
                    }
                }
        ...
        let drainedOnTimeout = drained.value
    ...
    func testCommitWithMatchingGenerationDrainsForReplayAndDisarmsTheTimeout() {
        let (wait, spy) = makeWait()
        // Lock-boxed: the timeout handler is `@Sendable`, so it cannot capture a
        // mutable local.
        let timeoutFired = LockedResult(false)
        queue.sync {
            wait.assumeIsolated { wait in
                _ = wait.beginWait(generation: 6) { _ in timeoutFired.value = true }
        ...
        let timedOut = timeoutFired.value
    ...
    func testAssumeIsolatedGivesSynchronousOnQueueAccess() {
        ...
        let (wait, _) = makeWait()
        queue.sync {
            wait.assumeIsolated { isolated in
                XCTAssertEqual(isolated.beginWait(generation: 1) { _ in }, [])
                XCTAssertEqual(isolated.enqueue(1, generation: 1), .queued(isFirst: true))
                XCTAssertEqual(isolated.drain(currentGeneration: 1), .replay([1], replayGeneration: 1))
                XCTAssertEqual(isolated.cancelWait(), [])
            }
        }
    }
}

/// Minimal lock-boxed result slot for values the `@Sendable` timeout handlers
/// write (tests only — same pattern as DeviceDNSCaptureRetryCycleTests' ManualClock).
