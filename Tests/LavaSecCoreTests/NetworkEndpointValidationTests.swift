...
final class NetworkEndpointValidationTests: XCTestCase {
    ...
    func testIPv4ScopeClassificationAcrossValidatorEntryPoints() {
        let cases: [(host: String, expected: ExpectedScope, note: String)] = [
            ...
            // NOTE: Class E (240.0.0.0/4) is NOT classified: only 255.0.0.0/8 is
            // reserved, so 240.0.0.0 through 254.255.255.255 classify as public.
            // Characterized as-is; flagged for review, not fixed here.
            ("240.0.0.0", .publicAddress, "Class E start is unclassified today (see NOTE)"),
            ("254.255.255.255", .publicAddress, "Class E below 255/8 is unclassified today (see NOTE)"),
    ...
    func testIPv6ScopeClassificationAcrossValidatorEntryPoints() {
        let cases: [(host: String, expected: ExpectedScope, note: String)] = [
            ...
            // NOTE: only the exact ::1 is loopback; every other low address in
            // the deprecated IPv4-compatible block classifies as public.
            ("::2", .publicAddress, "only ::1 is special-cased (see NOTE)"),
