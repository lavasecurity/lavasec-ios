...
package enum PinnedPublicHTTPSFetcher {
    ...
    package static func fetchResponse(
        ...
        try await fetchResponse(url: url, maximumByteCount: maximumByteCount, resolver: SystemHostResolver.resolve, configuration: .default)
    }
...
final class PinnedHTTPSExchange: @unchecked Sendable {
    ...
    /// Count of interim (1xx) heads discarded before the final response — bounds a pathological
    /// server that floods 1xx to keep the parser looping (see beginBody).
    private var interimResponseCount = 0
    private static let maximumInterimResponses = 8
    ...
    #if DEBUG
    /// Test seam (DEBUG only, never in release): drive the response parser with raw bytes on the
    /// internal queue WITHOUT opening a network connection, so header-framing edge cases (interim
    /// 1xx, chunked, redirects, EOF-terminated bodies) are unit-testable deterministically. Mirrors
    /// the live flow — `perform` sets the completion and the receive loop feeds `ingest` per chunk
    /// then calls `handleEndOfStream` on close.
    func drainResponseForTesting(
        _ chunks: [Data],
        completion: @escaping @Sendable (Result<PinnedPublicHTTPSFetcher.HTTPExchangeOutcome, Error>) -> Void
    ) {
        queue.async { [self] in
            self.completion = completion
            self.phase = .head
            do {
                for chunk in chunks where !isFinished {
                    try ingest(chunk)
                }
                if !isFinished {
                    try handleEndOfStream()
                }
            } catch {
    ...
    private func beginBody(headData: Data, leftover: Data) throws {
        ...
        // An interim 1xx (RFC 7230 §6.2) is NOT the final response — notably 103 Early Hints
        // (RFC 8297), which CDN-fronted blocklist sources legitimately emit on a GET. Treating it
        // as final would throw invalidHTTPStatus and that source would silently fail to sync.
        // Discard it and resume head parsing on the leftover bytes (which may already contain the
        // final head in the same packet); a connection close while still in .head fails closed
        // via handleEndOfStream. Bounded so a 1xx flood can't wedge the parser.
        if (100..<200).contains(head.statusCode) {
            interimResponseCount += 1
            guard interimResponseCount <= Self.maximumInterimResponses else {
                throw URLError(.badServerResponse)
            }
            statusCode = 0
            phase = .head
